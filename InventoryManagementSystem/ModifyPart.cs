...
namespace InventoryManagementSystem
{
    public partial class ModifyPart : Form
    {
        ...
        // Initialize -- overloaded with Outsourced part
        //      sets the form to display data from the selected part
        public void Initialize(Outsourced selectedPart)
        {
            
            OutsourcedRadio.Checked = true;

            machineCompanyNameIDTextBox.Text = selectedPart.CompanyName;
        ...
        // Initialize -- overloaded with Inhouse part
        //      sets the form to display data from the selected part
        public void Initialize(Inhouse selectedPart)
        {
            
            InHouseRadio.Checked = true;

            machineCompanyNameIDTextBox.Text = selectedPart.MachineID.ToString();
            idTextBox.Text = selectedPart.PartID.ToString();
            nameTextBox.Text = selectedPart.Name;
            inventoryTextBox.Text = selectedPart.InStock.ToString();
            priceTextBox.Text = selectedPart.Price.ToString();
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            ...
            tempID = tempPart.PartID;


            // if the outsourced radio box is checked create an Outsourced Part if not, create an Inhouse Part
            if (OutsourcedRadio.Checked)
            {
                //ManagementSystem.inventory.Parts.Remove(ManagementSystem.ActivePart);
                tempCompanyNameID = machineCompanyNameIDTextBox.Text;
                Outsourced tempPart = new Outsourced(tempCompanyNameID, tempID, tempName, tempPrice, tempInventory, tempMin, tempMax);
                ManagementSystem.Inventory.updatePart(tempID, tempPart);
                Reset();
            }
            else
            {
                ...
                //ManagementSystem.inventory.Parts.Remove(ManagementSystem.ActivePart);
                Inhouse tempPart = new Inhouse(tempMachineID, tempID, tempName, tempPrice, tempInventory, tempMin, tempMax);
                ManagementSystem.Inventory.updatePart(tempID, tempPart);
                Reset();
            }
            this.Hide();
